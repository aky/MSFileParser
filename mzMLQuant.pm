...
sub mzML_quant{
    ...
    foreach my $raw_input (@raw_files)
    {
        ...
        print $fh "Scan No.\tTitle\tRetention_time\tBase_Intensity\tPEPMASS\tCharge\tMSLevel\tPrevious Scan";
        ...
        print $fh1 "Scan No.\tTitle\tRetention_time\tBase_Intensity\tPEPMASS\tCharge\tMSLevel\tPrevious Scan";
...
sub Parse_mzML_spec{
    ...
    while($r=$r->next_elt())
    {
        ...
        if($r->gi eq 'precursor')# only present if mslevel 2/3
        {
    ...
    #print $spectra{MSLevel}," == MSLEVEL OUT\n";<>;
    if ($spectra{MSLevel}>1)
    {
        #print "\t",$spectra{MSLevel}," == MSLEVEL IN\n";<>;

        #print "\t","TITLE=>$spectra{TITLE}\n";<>;
        if (exists($spectra{compression}))
        {
            ...
            {
                #print "$spectra{TITLE}\n";
                #print "QC==>",join "|",@{$spectra{MS2}},"\n";
                my($spectra_hashref,$area_hashref,$rep_count)= spectrum_quantitator(\%spectra,\%params,\%plex);
                #print "QC==> ",$$spectra_hashref{TITLE},"\t\t\t",$$spectra_hashref{MSLevel},"\n";<>;                  
                if ($rep_count>=$params{min_reporters})
                {
                    print_spec($index,$spectra_hashref,$area_hashref,$fh);
                }                     
                        #}
            }
            else
            {
                print $fh1 "$index\t$spectra{TITLE}\t$spectra{RT}\t$spectra{INT}\t$spectra{MZ}\t$spectra{Z}\n";
            }
